...
export default function App() {
  ...
  const LeagueView = () => {
    const topUsers = globalLeaderboard.slice(0, 50);

    return (
      <div className="space-y-6 pb-20 animate-in fade-in duration-200">
        <div className="bg-slate-900 rounded-3xl p-6 text-white relative overflow-hidden shadow-xl mt-4">
          <div className="absolute -right-10 -top-10 opacity-10"><Trophy className="w-48 h-48" /></div>
          <h1 className="text-2xl font-black mt-3 mb-1">RANKING GLOBAL</h1>
          <p className="text-slate-400 text-sm flex items-center gap-2"><Users className="w-4 h-4" /> {users.length} Registrados</p>
        </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            {topUsers.length === 0 && <p className="p-4 text-center text-slate-500">Aún no hay usuarios.</p>}
            {topUsers.map((u, index) => (
              <div key={u.id} className={`flex items-center p-4 ${index !== topUsers.length - 1 ? 'border-b border-slate-50' : ''} ${u.id === user.id ? 'bg-indigo-50/50' : ''}`}>
                ...
                <div className="font-black text-slate-800 text-lg">{Math.floor(u.totalPoints)} <span className="text-xs font-normal text-slate-500">pts</span></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-md mx-auto bg-slate-50 min-h-screen relative shadow-2xl overflow-hidden flex flex-col">
        ...
        <nav className="bg-white border-t border-slate-100 flex justify-between px-8 py-4 pb-6 z-40 relative shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)]">
          ...
          <button onClick={() => setActiveTab('league')} className={`flex flex-col items-center gap-1 w-16 ${activeTab === 'league' ? 'text-indigo-600' : 'text-slate-400'}`}>
            <Trophy className={`w-6 h-6 ${activeTab === 'league' ? 'fill-indigo-100' : ''}`} /><span className="text-[10px] font-bold">Ranking</span>
          </button>
        </nav>
